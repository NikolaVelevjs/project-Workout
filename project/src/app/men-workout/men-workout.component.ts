...
@Component({
  selector: 'app-men-workout',
  templateUrl: './men-workout.component.html',
  styleUrls: ['./men-workout.component.css']
})
export class MenWorkoutComponent implements OnInit {
  workouts: any
  ...
  sort(Workouts: any[], program: string) {
    let sortedWorkouts = Workouts.filter(workout => workout.gender == "man")
    if (program == "All") {
      ...
      sortedWorkouts = sortedWorkouts.filter(workout => workout.program.toLowerCase() == program.toLowerCase())
